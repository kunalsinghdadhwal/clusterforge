...
pub fn load_and_process_data(file_path: &str, end_date: Option<&str>) -> crate::Result<RfmData> {
    ...
    let reference_date = DateTime::parse_from_rfc3339(end_date_str)?.with_timezone(&Utc);
...
fn prepare_features(
    df: DataFrame,
) -> crate::Result<(Array2<f64>, Vec<i64>, StandardScaler, Array2<f64>)> {
    ...
    let frequency: Vec<f64> = df.column("Frequency")?.f64()?.into_no_null_iter().collect();
...
mod tests {
    ...
    fn create_test_csv() -> NamedTempFile {
        ...
        writeln!(
            file,
            "536366,22633,HAND WARMER UNION JACK,6,12/01/2010 08:28:00,1.85,17850,United Kingdom"
        )
        .unwrap();
