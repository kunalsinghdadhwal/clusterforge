...
use clusterforge::{load_and_process_data, fit_kmeans, predict_cluster};
...
fn create_test_csv() -> NamedTempFile {
    ...
    writeln!(file, "536368,22752,SET 7 BABUSHKA NESTING BOXES,2,2011-12-05T10:15:00,7.65,12345,United Kingdom").unwrap();
...
fn test_rfm_computation() {
    ...
    for row in rfm_data.features.outer_iter() {
        for &value in row.iter() {
            ...
            assert!(value.abs() < 10.0, "Normalized value {} is out of expected range", value);
        }
    }
