...
pub fn fit_kmeans(
    ...
    if !(3..=5).contains(&n_clusters) {
        anyhow::bail!("Number of clusters should be between 3 and 5 for meaningful customer segmentation");
    }
    
    if rfm_data.features.nrows() < n_clusters {
        anyhow::bail!("Number of data points ({}) must be at least equal to number of clusters ({})", 
                     rfm_data.features.nrows(), n_clusters);
    }
...
mod tests {
    ...
    fn create_test_rfm_data() -> RfmData {
        ...
        let raw_features = Array2::from_shape_vec((4, 3), vec![
            1.0, 1.0, 100.0,
            30.0, 10.0, 1000.0,
            10.0, 5.0, 500.0,
            20.0, 3.0, 750.0,
