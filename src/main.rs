...
use clap::Parser;
use clusterforge::{Args, load_and_process_data, fit_kmeans, predict_cluster, viz};
use anyhow::Result;
...
fn run_prediction_mode(args: &Args, rfm_values: (f64, f64, f64)) -> Result<()> {
    ...
    println!("Input RFM values: R={}, F={}, M={}", 
    ...
    println!("  Size: {} customers ({:.1}% of total)", 
             cluster_sizes[cluster], cluster_percentage);
    println!("  Centroid (normalized): R={:.2}, F={:.2}, M={:.2}",
...
fn run_full_pipeline(args: &Args) -> Result<()> {
    ...
    let silhouette_score = model.compute_silhouette_sample(&rfm_data.features, 100.min(rfm_data.customer_ids.len()));
    ...
    println!("Cluster sizes saved to: {}", args.output.replace(".png", "_sizes.png"));
